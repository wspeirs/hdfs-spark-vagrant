Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", inline: <<-SHELL
    ...
    # install java
    apt install openjdk-8-jre-headless --yes
    sh -c "echo 'export JAVA_HOME=\"/usr/lib/jvm/java-8-openjdk-amd64\"' >> /etc/environment"
    ...
    mkdir .ssh
    cp /vagrant/id_rsa* /vagrant/known_hosts /vagrant/authorized_keys .ssh
